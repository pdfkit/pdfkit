class PDFKit
  # @return Generator class singleton instance
  def self.generator
    Generator.instance
  end
  ...
  class Generator
    ...
    # instanciate the class only once to create the singleton instance
    #
    # @return [Generator] class instance
    def self.instance
      @__pdfkit_generator__ ||= new
    end
    # builds the different parts of a document to generate a pdf out of it
    #
    # @param [String, Pathname] _pdf_document_path_or_name_ indicating the place where to store the pdf generated file or the desired document filename
    # @note use always .pdf extension in either full storage path or document name
    # @param [Hash] _document_parts_ with the document parts of the pdf to be generated
    #   - :title  => document title
    #   - :cover  => cover html string
    #   - :header => header html string
    #   - :body   => body html string
    #   - :footer => footer html string
    # @param [Hash] _document_configurations_ with the configurations of the pdf document
    ...
    # @note generated pdf document file path can be obtained by using path method on the returned File object
    def generate(_pdf_document_path_or_name_, _document_parts_, _document_configurations_, _stylesheets_paths_)
      ...
      # create environment to support pdfkit in generating the pdf document
      self.class.set_environment

      # inject files content must be called
      self.class.temporary_files_injection(_document_parts_[:cover], _document_parts_[:header], _document_parts_[:footer])
      # set the configurations for the pdf document to be generated
      _options = self.class.options_for_pdf_kit(_document_parts_, _document_configurations_) # files generated by previous method
      # generate the document pdf
      _document = self.class.print(_pdf_document_path, _options, _stylesheets_paths_)

      # delete environment that supported pdfkit in generating the pdf document
      self.class.unset_environment
    ...
    #
    # @param [String, Pathname] _pdf_document_path_ indicating the place where to store the pdf generated file
    # @note use always .pdf extension in either full storage path or document name
    ...
    private
      class << self
        ...
        #
        # @return [Pathname] path where generated pdf file is to be saved
        #
        def default_directory_path
          @__default_directory_path__ ||= PDFKit.configuration.default_options[:default_directory_path] || Pathname.new('documents')
        end
        # create the directory that will hold the generated pdf documents
        #
        # @return [Pathname] directory created for the generated pdf documents
        ...
        #
        # @return [Pathname] temporary directory path to use for pdfkit environment
        def temporary_directory_path
          @__temporary_directory_path__ ||= PDFKit.configuration.default_options[:support_directory_path] || Pathname.new('pdfkit')
        end
        ...
        def temporary_file_paths
          ...
          %W{cover header footer}.each do |file_name|
            _file_path = Pathname.new(temporary_directory_path).join("#{file_name}_support_file.html")
        ...
        # @raise [ArgumentError] when _pdf_document_path_or_name_ is not of type String or Pathname
        # @return [String] with path to document
        def document_path(_pdf_document_path_or_name_)
          ...
          return _pdf_document_path_or_name_.to_s if _pdf_document_path_or_name_.is_a? Pathname

          Pathname.new(default_directory_path).join(_pdf_document_path_or_name_).to_s
        end
        # generates a pdf document out of html files, except body part that must be a string
        #
        # The difference between this method print and generate method is that of the document parts are passed as HTML FILES
        ...
        def print(_pdf_document_path_, _options_, _stylesheets_paths_)
          # instanciate pdf kit and create the generated file
          ...
          _kit.to_file(_pdf_document_path_) # pdf is generated by this call
        end
      end
  end
end